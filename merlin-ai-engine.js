...
(function MerlinAIEngine() {
  ...
  async function sendTelegramNotify(userMessage) {
    var token  = localStorage.getItem('merlin_tg_token')  || '';
    var chatId = localStorage.getItem('merlin_tg_chatid') || '';
    if (!token || !chatId) return;  // 未配置，跳过

    var now = Date.now();
    // 同一个访客，5分钟内只发一次 Telegram 通知
    var shouldNotify = (now - _tgLastNotify) > 5 * 60 * 1000;
    // 但如果留了电话，无论如何都发
    ...
    var name  = STATE.customerName  || '未知';
    ...
    var text = [
      ...
      '👤 姓名：' + name,
      '📱 电话：' + phone,
      '💬 最新消息：' + userMessage.slice(0, 200),
      '🌐 语言：' + lang,
      '🕐 时间：' + time,
      '🔑 会话 ID：' + (STATE.sessionId || '-'),
      '',
      '👆 点此 WhatsApp 回复：https://wa.me/' + CFG.waNum
    ].join('\n');
