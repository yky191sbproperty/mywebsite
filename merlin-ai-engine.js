...
(function MerlinAIEngine() {
  ...
  // ══════════════════════════════════════════════════════
  // ★ TELEGRAM 设置（直接填在这里，无需 admin 后台）
  // ══════════════════════════════════════════════════════
  var TELEGRAM = {
    TOKEN:   '8866050732:AAHZnvFMI8KbLuBTo5zqAtHIGREZqWct0nM',
    CHAT_ID: '929531222'
  };
  // 获取方法：
  // 1) Telegram 搜索 @BotFather → /newbot → 复制 Token
  // 2) 搜索 @userinfobot → 发任意消息 → 复制 id 数字
  // ══════════════════════════════════════════════════════
  ...
  // ══════════════════════════════════════════════════════
  // ⑥ Claude API 调用（通过后端代理 / DeepSeek）
  //
  //  ⚠️  Anthropic API 不允许浏览器直接调用（CORS 限制）
  //  解决方案：
  //  A) 使用 DeepSeek API（直接支持浏览器调用）
  //  B) 使用自建后端代理（设置 MERLIN_PROXY_URL）
  //
  //  设置方法（merlin-admin.html → 设置 → 高级）：
  //  localStorage.setItem('merlin_proxy_url', 'https://your-proxy.com/v1/messages');
  // ══════════════════════════════════════════════════════
  async function callClaudeAPI(userMessage) {
    ...
    // ── 判断 Key 类型 ──
    var isDeepSeek  = apiKey.startsWith('sk-') && !apiKey.startsWith('sk-ant-');
    var isAnthropic = apiKey.startsWith('sk-ant-');
    var proxyUrl    = localStorage.getItem('merlin_proxy_url') || '';

    // ── DeepSeek：直接调用（支持 CORS）──
    if (isDeepSeek) {
      return callDeepSeekAPI(userMessage, systemPrompt, history, apiKey);
    }

    // ── Anthropic Claude：必须通过代理 ──
    if (isAnthropic) {
      if (!proxyUrl) {
        // 没有代理时，降级到预设回复并给提示
        console.warn('[Merlin AI] ⚠️  Claude API 需要后端代理。请在 admin → 设置 填写 merlin_proxy_url，或改用 DeepSeek API Key。');
        return getFallbackReply(userMessage);
      }
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(errData.error ? errData.error.message : 'Proxy error ' + response.status);
        }
        var data = await response.json();
        return (data.content && data.content[0] && data.content[0].text) || getFallbackReply(userMessage);
      } catch(e) {
        console.warn('[Merlin AI] Claude proxy error:', e.message);
        return getFallbackReply(userMessage);
      }
    }
  ...
  // ══════════════════════════════════════════════════════
  // ⑥b Telegram 通知（顾客发消息时自动通知 Rex）
  // ══════════════════════════════════════════════════════
  //
  //  设置方法（在 merlin-admin.html 控制台执行一次）：
  //  localStorage.setItem('merlin_tg_token', '你的BotToken');
  //  localStorage.setItem('merlin_tg_chatid', '你的ChatID');
  //
  //  如何获取：
  //  1. Telegram 搜索 @BotFather → /newbot → 复制 Token
  //  2. 搜索 @userinfobot → 发消息 → 复制 Chat ID
  // ══════════════════════════════════════════════════════
  var _tgLastNotify = 0;  // 防止同一顾客每条消息都发通知

  async function sendTelegramNotify(userMessage) {
    // 优先用代码里的 TELEGRAM 变量，其次用 localStorage（admin 后台设置）
    var token  = TELEGRAM.TOKEN  !== '填入你的BOT_TOKEN'  ? TELEGRAM.TOKEN  : (localStorage.getItem('merlin_tg_token')  || '');
    var chatId = TELEGRAM.CHAT_ID !== '填入你的CHAT_ID' ? TELEGRAM.CHAT_ID : (localStorage.getItem('merlin_tg_chatid') || '');
    if (!token || !chatId) {
      console.warn('[Merlin AI] ⚠️ Telegram 未配置，请在 merlin-ai-engine.js 顶部填写 TELEGRAM.TOKEN 和 TELEGRAM.CHAT_ID');
      return;
    }

    var now = Date.now();
    // 同一个访客，3分钟内只发一次通知（留电话时强制发）
    var shouldNotify = (now - _tgLastNotify) > 3 * 60 * 1000;
    var hasPhone = !!STATE.customerPhone;
    if (!shouldNotify && !hasPhone) return;

    _tgLastNotify = now;

    var name  = STATE.customerName  || '未知访客';
    var phone = STATE.customerPhone || '未提供';
    var lang  = STATE.lang === 'zh' ? '中文' : STATE.lang === 'ms' ? 'Melayu' : 'English';
    var time  = new Date().toLocaleString('zh-MY', { timeZone: 'Asia/Kuala_Lumpur' });
    ...
    var text = [
      ...
      '👤 姓名：' + name,
      '📱 电话：' + phone,
      '💬 消息：' + userMessage.slice(0, 300),
      '🌐 语言：' + lang,
      '📊 对话轮数：' + msgCount,
      '🕐 时间：' + time,
      '━━━━━━━━━━━━━━━━━━━━',
      '👆 WhatsApp 回复：https://wa.me/' + CFG.waNum
    ].join('\n');
    ...
    try {
      ...
      if (result.ok) {
        console.log('[Merlin AI] ✅ Telegram 通知已发送');
      } else {
        console.warn('[Merlin AI] Telegram 错误:', result.description);
      }
    } catch(e) {
      console.warn('[Merlin AI] Telegram 通知失败:', e.message);
    }
  }
